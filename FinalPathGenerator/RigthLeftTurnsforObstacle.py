import numpy as np
import Circle
def rightleftturn(x_main,y_main,waypointIndex,corner_value,path,data,directionChangeControl,next_direction,directionList,currentDirection):
    x_raidus=0
    y_radius=0
    center_x=0
    center_y=0
    ...
    if currentDirection=="K":
        ...
        if next_direction == "B":
            ...
            #hem radiusları hem merkez noktaları konuma göre yöne göre ayarlamak lazım.abs
            #şuanki aşamada Güneye ve doğuya hareket kadlı
            radius_x= abs(end_left_point-start_left_point)
            # print(x_raidus)
            radius_y = abs(end_up_point-start_down_point)
            center_x= start_left_point
            center_y= end_up_point
            starting_angle= np.pi
            ending_angle = np.pi/2
            #def drawingCircle(radius_x,radius_y,center_x,center_y,starting_angle,ending_angle):
            x_main, y_main = Circle.drawingCircle(radius_x,radius_y,center_x,center_y,starting_angle,ending_angle,x_main,y_main) 
    elif currentDirection=="G":
        #doğu yapıldı batı yapılmadı
        if next_direction == "B":
            radius_x= abs(end_right_point-start_right_point)
            # print(x_raidus)
            radius_y = abs(end_down_point-start_up_point)
            center_x= start_right_point
            center_y= end_down_point
            starting_angle= 2*np.pi
