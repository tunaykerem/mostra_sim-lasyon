...
import         cv2.imshow('Full HD RGB Camera (1920x1080)', frame)
        print("levha:", levha)
        
        # SUPER ROBUST dur levhası kontrolü - ÇOKLU KONTROL VE DEBUG
        print(f"🔍 DEBUG: levha='{levha}', len={len(str(levha)) if levha else 'None'}, type={type(levha)}")
        print(f"🔍 REPR: {repr(levha)}")
        
        # Çoklu kontrol yöntemi - hem exact match hem substring
        is_dur_sign = False
        
        if levha is not None:
            levha_str = str(levha).strip().lower()
            print(f"🔍 PROCESSED: '{levha_str}'")
            ...
            if (levha_str == "dur" or 
                ...
                str(levha) == "dur"):
                is_dur_sign = True
                print(f"🛑🛑🛑 DUR LEVHASI BULUNDU! Original: '{levha}', Processed: '{levha_str}' 🛑🛑🛑")
        
        print(f"🔍 FINAL RESULT: is_dur_sign={is_dur_sign}")
        ...
        if is_dur_sign:
            ...
            with stop_sign_lock:
                if not stop_sign_detected:
                    ...
                    print(f"🛑 DUR LEVHASI ZAMANLAYICI BAŞLADI!") cone distance calculation
try:
...
# Dur levhası için değişkenler
stop_sign_detected = False
stop_sign_timer = 0
stop_sign_duration = 3.0  # 3 saniye dur
stop_sign_lock = threading.Lock()
stop_sign_last_detection_time = 0  # Son tespit zamanı (tekrar tespit önlemek için)
stop_sign_cooldown = 5.0  # 5 saniye ara ile tekrar tespit edilebilir
...
def thread_function_1():
    ...
    global stop_sign_detected, stop_sign_timer, stop_sign_last_detection_time
    ...
    while not stop_event.is_set():
        ...
        if isinstance(result, tuple) and len(result) >= 2:
            ...
            print(f"🔍 YOLO SONUCU: levha='{levha}' (tip: {type(levha)}, None mi: {levha == 'None'})")
            
            # ÖNEMLİ: Dur levhası tespiti - hemen kontrol et
            if levha == "dur" or (levha and str(levha).lower() == "dur"):
                current_time = time.time()
                print(f"🎯🎯🎯 DUR LEVHASI ALGILANDI! levha='{levha}' 🎯🎯🎯")
                ...
                with stop_sign_lock:
                    print(f"🔒 Lock içindeyiz - stop_sign_detected: {stop_sign_detected}")
                    print(f"🕐 Son tespit zamanı: {stop_sign_last_detection_time}")
                    print(f"⏰ Cooldown süresi: {stop_sign_cooldown}")
                    print(f"⏱️ Geçen süre: {current_time - stop_sign_last_detection_time}")
                    
                    # Tekrar tespit önlemek için cooldown kontrolü
                    if not stop_sign_detected and (current_time - stop_sign_last_detection_time) > stop_sign_cooldown:
                        # İlk kez dur levhası tespit edildi
                        stop_sign_detected = True
                        stop_sign_timer = current_time
                        stop_sign_last_detection_time = current_time
                        print(f"🛑 DUR LEVHASI TESPİT EDİLDİ! Araç 3 saniye duracak...")
                        print(f"   Tespit edilen: '{levha}'")
                        print(f"   stop_sign_detected = {stop_sign_detected}")
                        print(f"   stop_sign_timer = {stop_sign_timer}")
                        print(f"   Global.breaking = {Global.breaking} olarak ayarlandı")
                    else:
                        print(f"⚠️ Dur levhası zaten tespit edilmiş veya cooldown aktif")
                        print(f"   stop_sign_detected: {stop_sign_detected}")
                        print(f"   cooldown check: {(current_time - stop_sign_last_detection_time) > stop_sign_cooldown}")
        else:
        ...
        # Dur levhası durumu görsel feedback
        with stop_sign_lock:
        ...
        if len(cone_positions) > 0:
            ...
            cone_y_position = 280 if stop_sign_detected else 250
        ...
        timestamp_y = 480 if stop_sign_detected else 430
